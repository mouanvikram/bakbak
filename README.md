...
> ⚠️ **Work in progress — single-instance only.** The backend core (auth, users, friends, chats, messages, attachments), the Socket.IO realtime layer, media/avatar uploads, and the React client are all implemented and wired end-to-end (~255 HTTP integration tests). Not yet built: horizontal scaling (no Socket.IO Redis adapter — realtime assumes one process), notifications, message reactions/replies, offline support, and any deployment/CI tooling. See [Status](#-current-status) below.
...
- **Auth** — register, login, email verification (+ resend), forgot/reset password, change password, logout, refresh-token rotation with revocation, per-account brute-force lockout. Argon2id hashing, SHA-256-hashed one-time tokens with expiry, anti-enumeration responses. Every access token carries a stable session id, so revoking a session (or logging out) also force-disconnects that session's live sockets.
...
- **Two-factor auth (email OTP)** — turning 2FA on requires confirming a 6-digit code emailed to the account; once on, every login pauses after the password check to email a fresh code and returns a short-lived challenge, exchanged for tokens via `POST /auth/login/verify-2fa` (with resend). Codes are single-use, hashed, 10-minute TTL.
- **Users** — profile read/update, **editable username with availability check** (email is read-only), avatar upload (incl. pre-signup), user search, delete account, coarse online / last-seen state. Bios are normalised to `null` or a real 10–500-char string.
