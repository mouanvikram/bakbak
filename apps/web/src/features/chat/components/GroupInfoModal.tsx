...
import { Camera, Check, Pencil, Plus, Search, UserMinus, X } from "lucide-react";
import type { ChatResponseType, FriendshipResponseType } from "@bakbak/contracts";
import {
  addParticipant,
  getChat,
  removeParticipant,
...
function personName(p: { username: string; profile?: { displayName?: string | null } | null }) {
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          <div className="flex flex-col items-center gap-3 px-5 py-6 text-center">
            <button
              ...
              className="relative rounded-full outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-500 disabled:cursor-default"
            >
              <Avatar name={chat.name || "Group"} src={chat.avatar} className="size-20" />
              {isAdmin && (
                <span className="absolute right-0 bottom-0 flex size-7 items-center justify-center rounded-full border-2 border-white bg-brand-500 text-white">
                  {busy === "photo" ? (
            ...
            {editingName ? (
              <div className="flex w-full items-center gap-2">
                <input
                  ...
                  className="h-10 min-w-0 flex-1 rounded-lg bg-slate-50 px-3 text-sm outline-none focus-visible:ring-2 focus-visible:ring-brand-500/30"
                />
              ...
              <div className="flex items-center gap-1.5">
                <h3 className="text-lg font-semibold text-slate-900">{chat.name}</h3>
                {isAdmin && (
                  <IconButton
            ...
            <div className="border-t border-gray-100 px-2 py-2">
              {!addOpen ? (
                <button
                  ...
                  className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-brand-600 transition-colors hover:bg-slate-50"
                >
                  <span className="flex size-9 items-center justify-center rounded-full bg-brand-500/10 text-brand-600">
